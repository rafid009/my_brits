...
# Project modules
from transformer.src.options import Options
from transformer.src.running import setup, pipeline_factory, validate, check_progress, NEG_METRICS
from transformer.src.datasets.data_utils import *
from transformer.src.utils import utils
from transformer.src.datasets.data import data_factory, Normalizer
from transformer.src.datasets.datasplit import split_dataset
from transformer.src.models.ts_transformer import model_factory
from transformer.src.models.loss import get_loss_module
from transformer.src.optimizers import get_optimizer

def add_season_id_and_save(data_folder, season_df, season_array=None, filename=None):
...
def run_transformer(config):
    ...
    if config['test_only'] != 'testset':
        ...
        logger.info('Running:\n{}\n'.format(' '.join(config)))  # command used to run
    ...
    if config['val_ratio'] > 0:
        # train_indices = [i for i in range(0, 32)]
        # val_indices = [32, 33]
        # test_indices = []
    ...
    if config['test_only'] == 'testset':  # Only evaluate and skip training
        ...
        _, _, predictions = test_evaluator.evaluate(keep_all=True, is_test=True)
...
# config = setup(params)  # configuration dictionary
# main(config)
