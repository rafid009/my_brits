...
from transformer.src.utils import utils, analysis
from transformer.src.models.loss import l2_reg_loss
from transformer.src.datasets.dataset import ImputationDataset, TransductionDataset, ClassiregressionDataset, collate_unsuperv, collate_superv
...
def setup(args):
    ...
    config = args  # configuration dictionary
    # print(config)
...
def validate(val_evaluator, tensorboard_writer, config, best_metrics, best_value, epoch):
    ...
    eval_start_time = time.time()
    with torch.no_grad():
        # p = val_evaluator.evaluate(epoch, keep_all=True)
        # print(f"p = {len(p)}")
        # print(p)
        aggr_metrics, per_batch, _ = val_evaluator.evaluate(epoch, keep_all=True)
    eval_runtime = time.time() - eval_start_time
...
class UnsupervisedRunner(BaseRunner):
    ...
    def evaluate(self, epoch_num=None, keep_all=True, is_test=False):
        ...
        if keep_all:
            return self.epoch_metrics, per_batch, predictions
        else:
            return self.epoch_metrics, None, predictions
