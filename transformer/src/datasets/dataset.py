...
class ImputationDataset(Dataset):
    ...
    def __init__(self, data, indices, mean_mask_length=3, masking_ratio=0.15,
                 mode='separate', distribution='geometric', exclude_feats=None, is_test=False):
        super(ImputationDataset, self).__init__()

        self.data = data  # this is a subclass of the BaseData class in data.py
        self.IDs = indices  # list of data IDs, but also mapping between integer index and ID
        self.feature_df = self.data.feature_df.loc[self.IDs]
        # print('num of nan:\n', self.feature_df.isna().sum())
        self.original_mask = self.data.original_mask.loc[self.IDs]
    ...
    def __getitem__(self, ind):
        ...
        X = np.array(self.feature_df.loc[self.IDs[ind]].values, dtype=np.float32)  # (seq_length, feat_dim) array
        # print('X type: ', type(X))
        # print('X isna: ', np.isnan(X))
        X_mask = np.array(self.original_mask.loc[self.IDs[ind]].values)
        if not self.is_test:
            mask = noise_mask(X, self.masking_ratio, self.mean_mask_length, self.mode, self.distribution,
                            self.exclude_feats, X_mask)  # (seq_length, feat_dim) boolean array
        else:
            mask = X_mask
...
def noise_mask(X, masking_ratio, lm=3, mode='separate', distribution='geometric', exclude_feats=None, orig_mask=None):
    """
    ...
    mask = np.logical_xor(mask, orig_mask)
    return mask
