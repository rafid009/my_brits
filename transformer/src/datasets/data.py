...
class SeasonData(BaseData):
    ...
    def __init__(self, root_dir, file_list=None, pattern=None, n_proc=1, limit_size=None, config=None):
        ...
        self.all_df = self.load_all(root_dir, file_list=file_list, pattern=pattern)
        self.all_df = self.all_df.sort_values(by=['season_id'])  # datasets is presorted
        self.all_df = self.all_df.set_index('season_id')
        self.original_mask = pd.DataFrame(self.all_df)
        self.original_mask[self.all_df.columns.values.tolist()] = np.isnan(self.all_df)
        self.all_df = self.all_df.fillna(0)
        self.all_IDs = self.all_df.index.unique()  # all sample (session) IDs
        # print('all ids: ', self.all_IDs)
        ...
        self.feature_names = features#['feed_speed', 'current', 'voltage', 'motor_current', 'power']
        self.feature_df = self.all_df[self.feature_names]
