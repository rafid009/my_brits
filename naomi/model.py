...
class NAOMI(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.stochastic = False#(self.task == 'basketball')
    ...
    def forward(self, data, ground_truth, missing_mask, batch_size=16):
        ...
        for t in range(data.shape[0]):
            ...
            for l, dim in self.dims.items():
                ...
                if t + 2 * step_size <= data.shape[0]:
                    next_miss_t = missing_mask[t+step_size]
                    ...
                    if self.stochastic:
                        ...
                        loss += nll_gauss(dec_mean_t, dec_std_t, next_t, next_miss_t)
                    else:
                        loss += torch.sum(((dec_mean_t - next_t) * next_miss_t).pow(2))
