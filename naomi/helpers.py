...
def run_epoch(train, model, exp_data, clip, optimizer=None, batch_size=16, num_missing=None, teacher_forcing=True):
    losses = []
    ...
    while i + batch_size <= exp_data.shape[0]:
        ...
        if teacher_forcing:
            batch_loss = model(data, ground_truth, missing_mask, batch_size)
        else:
            data_list = []
...
def collect_samples_interpolate(policy_net, expert_data, use_gpu, i_iter, task, size=16, name="sampling_inter", draw=False, stats=False, num_missing=None):
    ...
    samples = policy_net.sample(data_list, size)
    
    states = samples[:-1, :, :]
    actions = samples[1:, :, :]
    exp_states = ground_truth[:-1, :, :]
    exp_actions = ground_truth[1:, :, :]
    states_missing = missing_mask[:-1, :, :]
    actions_missing = missing_mask[1:, :, :]
    ...
    return exp_states.data, exp_actions.data, ground_truth.data, states, actions, samples.data, states_missing.data, actions_missing.data
